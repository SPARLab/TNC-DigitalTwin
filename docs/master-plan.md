...
**Last Updated:** February 27, 2026 (CON-INAT-11 complete)  
...
| Metric | Value |
|--------|-------|
| **Active scope** | 165 / 185 complete (**89%**) |
...
| 1 | iNaturalist | 🟡 In Progress | 14 | 2 | 0 | 0 | 1 | 17 | `v2/inaturalist` |
...
- **2D/3D View Mode Toggle** — Implemented. MapContainer supports MapView (2D) and SceneView (3D) with unified `MapControlRail` (top-right): 2D/3D text toggle, LiDAR visibility (3D only), zoom +/−, compass. All 2D layers drape onto terrain in 3D; LiDAR PointCloudLayer loads in 3D only. Camera preserved across toggles via `useArcgisViewLifecycle`. See `MapContext.viewMode`, `MapControlRail`, REF-15, REF-16.
...
| Map↔sidebar detail smoke checks | ✅ Implemented | Cursor | Feb 25 | REF-08D: Manual smoke checks executed for DataONE, Dendra, TNC ArcGIS: back nav, map focus/recenter, save/pin, selection retention validated. REF-08 complete; Phase 12: 10/12 tasks complete. Checklist documented in phase-12-ai-refactor.md. |
...
| CalFlora layer soft-hidden from catalog | ✅ Implemented | Cursor | Feb 27 | CON-INAT-11: CalFlora temporarily removed from left-sidebar catalog (EXTERNAL_LAYERS in layerRegistry) due to licensing ambiguity. Custom sidebar/map/adapter logic retained for future re-enable. CON-INAT-13 tracks licensing-safe media filter before reactivation. See phase-1-inaturalist.md, phase-8-calflora.md. |
